...
const Stores = () => {
  ...
  const fetchStores = async () => {
    try {
      ...
      const storesData = normalizeArrayResponse(storesResponse);
  ...
  const categories = useMemo(() => {
    const storeCategories = stores.map((store) => store.category);
    ...
    const uniqueCategories = [...storeCategories, ...productCategories]
      .filter(Boolean)
  ...
  const filteredStores = useMemo(() => {
    ...
    return stores.filter((store) => {
      const storeProducts = productsByStoreId[store.id] || [];
      const productText = storeProducts
        .map((product) => {
          const categoryName =
            typeof product.category === "string"
              ? product.category
      ...
      const matchesSearch =
        ...
        productText.toLowerCase().includes(query);
  ...
  }, [stores, productsByStoreId, searchQuery, selectedCategory]);
  ...
  return (
    <main className="min-h-screen bg-[#f6f1e7] text-emerald-950">
      <div className="mx-auto max-w-7xl px-6 py-8 lg:px-8">
        <section className="overflow-hidden rounded-[2.5rem] bg-emerald-950 p-8 text-white shadow-[0_30px_100px_rgba(8,28,21,0.2)] md:p-10">
          <div className="grid gap-8 lg:grid-cols-[1fr_0.75fr] lg:items-center">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-lg leading-8 text-white/70">
                Search stores by seller, product type, category, or location and find products from trusted Shoplinker vendors.
              </p>
            </div>
            ...
            <div className="rounded-[2rem] bg-white p-5 text-emerald-950">
              ...
              <div className="relative mt-4">
                ...
                <input
                  type="text"
                  placeholder="Search by product type, category, store, or location..."
                  value={searchQuery}
        ...
        {stores.length === 0 ? (
          ...
          <section className="mt-8 rounded-[2.5rem] border border-emerald-950/10 bg-white/85 p-12 text-center shadow-sm">
            ...
            <p className="mx-auto mt-3 max-w-xl font-semibold leading-7 text-emerald-950/60">
              Try a different product type, category, store name, or location.
            </p>
            <button
              type="button"
              onClick={() => {
          ...
          <section className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredStores.map((store) => (
              ...
              >
                <div className="relative h-44 w-full overflow-hidden bg-[#f6f1e7]">
                  {store.banner ? (
                    <img
                      src={store.banner}
                ...
                <div className="p-6">
                  ...
                  <p className="line-clamp-3 font-semibold leading-7 text-emerald-950/65">
                    {store.description || "No description added yet."}
                  </p>

                  {(productsByStoreId[store.id] || []).length > 0 && (
                    <div className="mt-5 flex flex-wrap gap-2">
                      {(productsByStoreId[store.id] || [])
                        .slice(0, 4)
                        .map((product) => {
                          const categoryName =
                            typeof product.category === "string"
                          ...
                          return (
                            <span
                              key={product.id}
                              className="rounded-full bg-lime-100 px-3 py-1 text-xs font-black text-emerald-950"
                            >
